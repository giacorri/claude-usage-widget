...
# OpenAI's mark is monochrome, so Codex gets plain white next to the orange.
CODEX_MARK_COLOR = "#ececec"
# Gap between the Claude block and the Codex block in bars view, so the two
# pairs read as two groups and not as four rows of one thing.
CODEX_BLOCK_GAP = 8
...
# OpenAI's blossom, as published by Simple Icons (CC0). Kept as path data
# rather than drawn with strokes like the Claude mark: the woven lobes are
# what make it recognisable, and there is no stroke construction that keeps
# them at 20 pt.
_OPENAI_MARK_PATH = (
    "M22.2819 9.8211a5.9847 5.9847 0 0 0-.5157-4.9108 6.0462 6.0462 0 0 0-6.5098-2.9"
    "A6.0651 6.0651 0 0 0 4.9807 4.1818a5.9847 5.9847 0 0 0-3.9977 2.9 6.0462 6.0462 0 0 0 "
    ".7427 7.0966 5.98 5.98 0 0 0 .511 4.9107 6.051 6.051 0 0 0 6.5146 2.9001"
    "A5.9847 5.9847 0 0 0 13.2599 24a6.0557 6.0557 0 0 0 5.7718-4.2058 5.9894 5.9894 0 0 0 "
    "3.9977-2.9001 6.0557 6.0557 0 0 0-.7475-7.0729zm-9.022 12.6081a4.4755 4.4755 0 0 1"
    "-2.8764-1.0408l.1419-.0804 4.7783-2.7582a.7948.7948 0 0 0 .3927-.6813v-6.7369l2.02 "
    "1.1686a.071.071 0 0 1 .038.052v5.5826a4.504 4.504 0 0 1-4.4945 4.4944zm-9.6607-4.1254"
    "a4.4708 4.4708 0 0 1-.5346-3.0137l.142.0852 4.783 2.7582a.7712.7712 0 0 0 .7806 0"
    "l5.8428-3.3685v2.3324a.0804.0804 0 0 1-.0332.0615L9.74 19.9502a4.4992 4.4992 0 0 1"
    "-6.1408-1.6464zM2.3408 7.8956a4.485 4.485 0 0 1 2.3655-1.9728V11.6a.7664.7664 0 0 0 "
    ".3879.6765l5.8144 3.3543-2.0201 1.1685a.0757.0757 0 0 1-.071 0l-4.8303-2.7865"
    ...
    "-.407-.667zm2.0107-3.0231l-.142-.0852-4.7735-2.7818a.7759.7759 0 0 0-.7854 0"
    "L9.409 9.2297V6.8974a.0662.0662 0 0 1 .0284-.0615l4.8303-2.7866a4.4992 4.4992 0 0 1 "
    "6.6802 4.66zM8.3065 12.863l-2.02-1.1638a.0804.0804 0 0 1-.038-.0567V6.0742a4.4992 "
    "4.4992 0 0 1 7.3757-3.4537l-.142.0805L8.704 5.459a.7948.7948 0 0 0-.3927.6813zm1.0976"
    "-2.3654l2.602-1.4998 2.6069 1.4998v2.9994l-2.5974 1.4997-2.6067-1.4997Z"
)
_codex_mark_renderers: dict[str, Any] = {}


def draw_codex_mark(p: QPainter, cx: float, cy: float, size: float,
                    color: str = CODEX_MARK_COLOR) -> None:
    """Draw the OpenAI blossom at (cx, cy), *size* across — Codex's mark."""
    from PySide6.QtSvg import QSvgRenderer

    renderer = _codex_mark_renderers.get(color)
    if renderer is None:
        svg = (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">'
               f'<path fill="{color}" d="{_OPENAI_MARK_PATH}"/></svg>')
        renderer = QSvgRenderer(QByteArray(svg.encode("utf-8")))
        _codex_mark_renderers[color] = renderer
...
class UsageOverlay(QWidget):
    ...
    def _apply_size(self) -> None:
        ...
        if self._view_mode == VIEW_MODE_GAUGE:
            ...
            if self._codex_available:
                base += 2 * SCOPED_ROW_HEIGHT + CODEX_BLOCK_GAP  # Codex pair
    ...
    def _paint_full(self, p: QPainter, w: int, h: int) -> None:
        ...
        # what the mark says in a gutter. Each provider's mark sits centred
        # against its own pair of rows, which is also what keeps the panel two
        # rows tall.
        gutter = 24 * s
        row_pad_x = pad_x + gutter
        bar_w = w - row_pad_x - pad_x
        mark_x = pad_x + gutter * 0.40
        row_pitch = 15 * s + bar_h + 10 * s
        claude_rows = 3 if self._scoped_label else 2

        # --- Session row ---
        y = pad_y + 2 * s
        # The mark is centred on the block's bars, not its box: the row's
        # label sits above its bar, so the optical middle is a little low.
        draw_claude_mark(
            p, mark_x, y + 10 * s + (claude_rows - 1) * row_pitch / 2, 21 * s)
        self._draw_row(
        ...
        # --- Codex rows — a second Session/Weekly pair under its own mark ---
        if self._codex_available:
            y2 += CODEX_BLOCK_GAP * s
            draw_codex_mark(p, mark_x, y2 + row_pitch + 10 * s + row_pitch / 2, 19 * s)
            for label, pct, reset_ts in (
                ("Session", self._codex_session_pct, self._codex_session_reset),
                ("Weekly", self._codex_weekly_pct, self._codex_weekly_reset),
            ):
                y2 = y2 + row_pitch
                self._draw_row(
                    p, y2, w, row_pad_x, bar_w, bar_h, bar_r, font_label, font_small,
